...
Document Updated At: 2026-09-05T15:40:00+08:00
...
│   └── README.md             Inbox 使用规则
└── README.md
```

- `01_Objects/`：存放正式收录的对象；对象类型通过数据字段表达，不通过文件夹分类。
- `02_Relations/`：存放对象之间正式记录的关系。
- `Inbox/`：辅助入口，不是第三个 Canonical 类别；所有尚未完成正式接纳流程的新对象，以及与接纳流程直接相关的结构化证据，都应先在这里处理。
...
- `Inbox/README.md`：定义维护者和 Agent 应如何使用 Inbox，以及什么条件下内容才能进入正式 `01_Objects/` / `02_Relations/`。
